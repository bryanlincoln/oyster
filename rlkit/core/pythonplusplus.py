...
def dict_of_list_to_list_of_dicts(dict, n_items):
    """
...
def list_of_dicts_to_dict_of_lists(lst, keys_to_include=None):
    """
    x = [
        {'foo': 3, 'bar': 1},
        {'foo': 4, 'bar': 2},
    ...
    if len(lst) == 0:
        return {}
    keys = [key for key in lst[0].keys() if key in keys_to_include] if keys_to_include is not None else lst[0].keys()
...
class IntIdDict(collections.defaultdict):
    ...
    def _init_(self, **kwargs):
        c = itertools.count()
        self.inverse_dict = {}
        super()._init_(lambda: next(c), **kwargs)

    def _getitem_(self, y):
        int_id = super()._getitem_(y)
...
class ConditionTimer(object):
    ...
    def _init_(self, trigger_period):
        """
