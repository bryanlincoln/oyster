...
class MultiTaskReplayBuffer(object):
    ...
    def add_sample(self, task, observation, action, reward, terminal, next_observation, **kwargs):

        if isinstance(self._action_space, Discrete):
            action = np.eye(self._action_space.n)[action]
        self.task_buffers[task].add_sample(observation, action, reward, terminal, next_observation, **kwargs)
...
def get_dim(space):
    if isinstance(space, Box):
        if len(space.shape) == 3:  # images' structure will be kept
            return space.shape
